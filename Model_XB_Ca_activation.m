function [ dYdT, dSL, F_XB, F_passive ] = Model_XB_Ca_activation( t,y,para, var)
Lsref = 1.9;
TmpC = para(1);
MgATP = para(2);
MgADP = para(3);
...
% mod(t,stim_f)
% Ca_i = spline(T'/max(T),Ca',mod(t,stim_f)/stim_f);
...
% k_on = var(4)*1000; % Campbell et al, Biophysical Journal 2018
% k_off =var(5)*4000; % manually tuned parameter!
% 
...
% kd      = var(10)*2.0909e+03;% myosin-actin detach rate constant, 1/sec
% k1      = var(11)*152.0917; % transition A1 to A2 rate constant, 1/sec
% k_1     = var(12)*33.5885; % transition A2 to A1 rate constant, 1/sec
% k2      = var(13)*648.1694; % transition A2 to A3 rate constant, 1/sec
% k_2     = var(14)*33.7370;% transition A3 to A2 rate constant, 1/sec
% k3      = var(15)*119.8460; % transition A3 to P rate constant, 1/sec
% alpha1  = 10.0; % Stretch sensing parameter for k1 and k?1, 1/um
% alpha2  = 9.1; % Stretch sensing parameter for k2 and k?2, 1/um
% alpha3  = var(16)*0.5*59.3; % Stretch sensing parameter for k3, 1/um
% s3      = 9.9e-3;  % Strain at which k3 is minimum, um
...
% K_coop = 0.32*var(2); % Campbell et al, Biophysical Journal 2018,
% k_on = 102.8*var(3);% Campbell et al, Biophysical Journal 2018
% k_off = 2752*var(4); % manually tuned parameter!
